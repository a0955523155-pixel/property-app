...
const ProjectEditor = ({ initialData, onSave, onBack, agencyDB }) => {
  ...
  const [projectTeam, setProjectTeam] = useState(initialData.projectTeam || {
    agency: "", broker: "", 
    developer: "", developerType: "general", developerNo: "", developerImg: null, 
    marketer: "", marketerNo: "", marketerImg: null, 
    scrivener: ""
  });
  ...
  const defaultHandover = {
    handoverDate: "", remotes: "0", keysFront: "0", keysBack: "0", warranty: false, drawings: false, electricityBill: "", waterBill: "", originalPermit: false
  };
  ...
  const tempLandTotals = useMemo(() => {
    ...
    tempLand.items.forEach(item => {
       ...
       const hM2 = item.detailM2 ? Number(item.detailM2) : (rawM2 * (num / denom));
       const price = Number(item.subtotal) || 0;
       sumM2 += hM2;
       sumPing += item.detailPing ? Number(item.detailPing) : toPing(hM2);
       sumPrice += price;
    });
  ...
  const sortedBuildings = useMemo(() => {
      return [...buildings].sort((a, b) => (a.address || "").localeCompare(b.address || "", "zh-Hant"));
  }, [buildings]);

  // --- 初始化與儲存 ---
  useEffect(() => { 
      if (lands.length > 0 && visibleLandIds.length === 0) setVisibleLandIds(lands.map(l => l.id)); 
      if (lands.length > visibleLandIds.length) { 
          const newIds = lands.map(l => l.id); 
          const currentIds = new Set(visibleLandIds); 
          const added = newIds.filter(id => !currentIds.has(id)); 
          if (added.length > 0) setVisibleLandIds(prev => [...prev, ...added]); 
      } 
  }, [lands]);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      const rawData = { 
        ...
        projectTeam, buyers, lands, buildings, transactions, handoverData, requisitions, 
        updatedAt: new Date().toISOString() 
      };
      onSave(deepSanitize(rawData));
    }, 1500);
    return () => clearTimeout(timer);
  }, [projectName, projectZone, projectSite, projectTeam, buyers, lands, buildings, transactions, handoverData, requisitions]);
  ...
  const handleAgencyChange = (agencyName) => { const brokers = (agencyDB && agencyDB[agencyName]) || []; const autoBroker = brokers.length === 1 ? brokers[0] : ""; setProjectTeam({ ...projectTeam, agency: agencyName, broker: autoBroker }); };
  ...
  const saveBuilding = () => { 
      ...
      setTempBuilding({ 
          saleDate: new Date().toISOString().split('T')[0], // Reset Date
          permitNumber: "", address: "", license: "", buildNumber: "", areaM2: "", pricePerUnit: "", totalPrice: "", sellers: [], permitImage: null, licenseImage: null, buildNoImage: null 
      }); 
  ...
  const handleExport = () => exportMasterCSV(projectName, buyers, lands, buildings, transactions, handoverData, projectTeam, requisitions, visibleLandIds);
