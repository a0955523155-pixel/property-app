...
import { 
  ...
  Calculator, Minus, Link as LinkIcon, ClipboardCheck, Key, FileText, Calendar as CalendarIcon, Tag, Building2, Settings, Briefcase, Filter, Eye, EyeOff
} from 'lucide-react';
...
const ProjectEditor = ({ initialData, onSave, onBack, agencyDB }) => {
  ...
  const [tempBuilding, setTempBuilding] = useState({permitNumber: "", address: "", license: "", buildNumber: "", areaM2: "", pricePerUnit: "", totalPrice: "", sellers: [], permitImage: null, licenseImage: null, buildNoImage: null});
  ...
  // --- 計算邏輯 (略) ---
  const landGrandTotal = useMemo(() => { let totalAreaM2 = 0, totalAreaPing = 0, totalMoney = 0; lands.forEach(l => { totalAreaM2 += Number(l.holdingAreaM2) || 0; totalAreaPing += Number(l.holdingAreaPing) || 0; totalMoney += Number(l.totalPrice) || 0; }); return { m2: totalAreaM2.toFixed(3), ping: totalAreaPing.toFixed(3), price: totalMoney }; }, [lands]);
  const stats = useMemo(() => { let totalIncome = 0, totalExpense = 0; const subTotals = { general: { income: 0, expense: 0 }, land: { income: 0, expense: 0 }, building: { income: 0, expense: 0 } }; transactions.forEach(t => { const val = Number(t.amount) || 0; const lType = t.linkedType || 'general'; if (t.type === 'income') { totalIncome += val; if (subTotals[lType]) subTotals[lType].income += val; } else { totalExpense += val; if (subTotals[lType]) subTotals[lType].expense += val; } }); const netProfit = totalIncome - totalExpense; const roi = totalExpense > 0 ? ((netProfit / totalExpense) * 100).toFixed(2) : 0; return { totalIncome, totalExpense, netProfit, roi, subTotals }; }, [transactions]);
  const groupedTransactions = useMemo(() => { const groups = { general: [], land: [], building: [] }; const sortedTx = [...transactions].sort((a,b) => new Date(a.date) - new Date(b.date)); sortedTx.forEach(t => { const type = t.linkedType || 'general'; if (groups[type]) groups[type].push(t); }); return groups; }, [transactions]);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      const rawData = { 
        ...
        projectTeam, buyers, lands, buildings, transactions, handoverData, updatedAt: new Date().toISOString() 
      };
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-gray-50 p-6 rounded-2xl border border-gray-100">
                ...
                <div>
                   ...
                   >
                      ...
                      {agencyDB && Object.keys(agencyDB).map(ag => <option key={ag} value={ag}>{ag}</option>)}
                   </select>
                </div>
        ...
        {activeTab === 'finance' && (
           <div className="space-y-8 animate-fadeIn">
              ...
              <div className="bg-white p-8 rounded-3xl shadow-sm border border-gray-200">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  {['general', 'land', 'building'].map(key => {
                     const bg = key==='general' ? 'bg-gray-50 border-gray-200 text-gray-700' : key==='land' ? 'bg-blue-50 border-blue-100 text-blue-800' : 'bg-orange-50 border-orange-100 text-orange-800';
                     const label = key==='general' ? '一般專案收支' : key==='land' ? '土地出售人帳目' : '建物出售人帳目';
                     const Icon = key==='general' ? DollarSign : key==='land' ? Map : Home;
                     const profit = (stats.subTotals[key]?.income||0) - (stats.subTotals[key]?.expense||0);
                     return (
                        <div key={key} className={`p-6 rounded-2xl border-2 ${bg}`}>
                           <div className="font-black mb-4 flex items-center gap-2 text-sm uppercase tracking-widest"><Icon className="w-5 h-5"/> {label}</div>
                           <div className="space-y-3 text-sm">
                              <div className="flex justify-between"><span>收入：</span><span className="font-mono font-bold text-green-600">${(stats.subTotals[key]?.income || 0).toLocaleString()}</span></div>
                              <div className="flex justify-between"><span>支出：</span><span className="font-mono font-bold text-red-500">${(stats.subTotals[key]?.expense || 0).toLocaleString()}</span></div>
                              <div className="flex justify-between border-t border-black/10 pt-3 mt-3 font-black"><span>淨利：</span><span className={`font-mono ${profit >= 0 ? 'text-blue-600' : 'text-orange-600'}`}>${profit.toLocaleString()}</span></div>
                           </div>
                        </div>
                     )
                  })}
              ...
              {/* ✅ 勾選顯示控制面板 (Checkbox) */}
              <div className="bg-gray-50 border border-gray-200 rounded-xl p-4 flex flex-wrap gap-6 items-center">
                 <span className="text-xs font-black text-gray-400 uppercase tracking-widest flex items-center gap-1"><Filter className="w-4 h-4"/> 顯示篩選:</span>
                 <label className="flex items-center gap-2 cursor-pointer font-bold text-gray-700 select-none hover:text-black">
                    <input type="checkbox" className="w-5 h-5 accent-gray-700 rounded" checked={visibleLedgers.general} onChange={(e)=>setVisibleLedgers({...visibleLedgers, general: e.target.checked})} />
                    <span>一般專案收支</span>
                 </label>
                 <label className="flex items-center gap-2 cursor-pointer font-bold text-blue-700 select-none hover:text-blue-900">
                    <input type="checkbox" className="w-5 h-5 accent-blue-600 rounded" checked={visibleLedgers.land} onChange={(e)=>setVisibleLedgers({...visibleLedgers, land: e.target.checked})} />
                    <span>土地出售人帳目</span>
                 </label>
                 <label className="flex items-center gap-2 cursor-pointer font-bold text-orange-700 select-none hover:text-orange-900">
                    <input type="checkbox" className="w-5 h-5 accent-orange-500 rounded" checked={visibleLedgers.building} onChange={(e)=>setVisibleLedgers({...visibleLedgers, building: e.target.checked})} />
                    <span>建物出售人帳目</span>
                 </label>
              </div>

              {/* ✅ 直向排列的交易列表 (Vertical Stack) */}
              <div className="space-y-8">
                 {/* 一般專案 */}
                 {visibleLedgers.general && (
                    <TransactionTable 
                       data={groupedTransactions.general} 
                       typeLabel="一般專案收支明細" 
                       colorTheme="border-gray-200 shadow-sm"
                       subStats={stats.subTotals.general} 
                    />
                 )}
                 ...
                 {visibleLedgers.land && (
                    <TransactionTable 
                       data={groupedTransactions.land} 
                       typeLabel="土地出售人帳目明細" 
                       colorTheme="border-blue-200 shadow-sm" 
                       subStats={stats.subTotals.land}
                    />
                 )}

                 {/* 建物 */}
                 {visibleLedgers.building && (
                    <TransactionTable 
                       data={groupedTransactions.building} 
                       typeLabel="建物出售人帳目明細" 
                       colorTheme="border-orange-200 shadow-sm" 
                       subStats={stats.subTotals.building}
                    />
                 )}
                 
                 {/* 如果全都不勾選的提示 */}
                 {!visibleLedgers.general && !visibleLedgers.land && !visibleLedgers.building && (
                    <div className="p-12 text-center text-gray-400 border-2 border-dashed rounded-xl italic">
                       已隱藏所有帳目，請勾選上方類別以檢視。
                    </div>
                 )}
              </div>
