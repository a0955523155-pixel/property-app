...
import { 
  ...
  Calculator, Minus, Link as LinkIcon, ClipboardCheck, Key, FileText, Calendar as CalendarIcon, Tag, Building2, Settings, Briefcase, Filter, Eye, EyeOff, Receipt
} from 'lucide-react';
...
const ProjectEditor = ({ initialData, onSave, onBack, agencyDB }) => {
  ...
  const [activeTab, setActiveTab] = useState('requisition'); // 方便測試，預設跳到請款單
  ...
  const [printConfig, setPrintConfig] = useState({
    team: true, buyers: true, lands: true, buildings: true, handover: true, finance: true, requisition: true // ✅ 新增 requisition
  });
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      const rawData = { 
        ...
        projectTeam, buyers, lands, buildings, transactions, handoverData, 
        requisitions, // ✅ 儲存請款單
  ...
  const addRequisition = () => {
    ...
    setNewReq({ ...newReq, details: "", shareholder: "", amount: "" }); // 保留日期與標的，方便連續輸入
  };
  ...
  const handleLandItemChange = (idx, field, value) => { const newItems = [...tempLand.items]; newItems[idx][field] = value; if (['areaM2', 'shareNum', 'shareDenom', 'pricePerPing'].includes(field)) { const area = Number(newItems[idx].areaM2)||0, num = Number(newItems[idx].shareNum)||0, denom = Number(newItems[idx].shareDenom)||1, price = Number(newItems[idx].pricePerPing)||0; const hM2 = area * (num / denom); newItems[idx].subtotal = Math.round(toPing(hM2) * price).toString(); } setTempLand({ ...tempLand, items: newItems }); };
  const saveLand = () => { if (tempLand.items.some(item => !item.lotNumber)) return alert("請填寫所有地號"); let totalM2=0, totalPingSum=0, totalPriceSum=0; tempLand.items.forEach(item => { const hM2 = Number(item.areaM2)*(Number(item.shareNum)/Number(item.shareDenom)); totalM2+=hM2; totalPingSum+=toPing(hM2); totalPriceSum+=(Number(item.subtotal)||0); }); const landData = { ...tempLand, holdingAreaM2: totalM2.toFixed(3), holdingAreaPing: totalPingSum.toFixed(3), totalPrice: totalPriceSum }; if (editingLandId) setLands(lands.map(l => l.id === editingLandId ? { ...landData, id: l.id } : l)); else setLands([...lands, { ...landData, id: Date.now() }]); setTempLand({ section: "", items: [createEmptyLandItem()], sellers: [] }); setShowLandForm(false); setEditingLandId(null); };
  ...
  // ✅ 匯出參數增加 requisitions
  const handleExport = () => exportMasterCSV(projectName, buyers, lands, buildings, transactions, handoverData, projectTeam, requisitions);
    ...
    <div className="animate-fadeIn pb-24 text-base app-wrapper">
      <div className="print:hidden">
        ...
        <div className="flex gap-3 mb-8 overflow-x-auto no-scrollbar">
          ...
          <button onClick={() => setActiveTab('requisition')} className={`flex items-center gap-2 px-6 py-3 rounded-xl font-bold whitespace-nowrap transition-all duration-200 text-sm ${activeTab === 'requisition' ? 'bg-blue-600 text-white shadow-md' : 'bg-white text-gray-600 hover:bg-gray-100 border'}`}><Receipt className="w-5 h-5" /> 請款單</button>
        ...
        {/* 0. 專案團隊 */}
        {activeTab === 'team' && (
          <div className="bg-white rounded-2xl shadow-sm border p-8 animate-fadeIn"><h2 className="font-bold text-gray-700 mb-6 flex items-center gap-2 border-l-4 border-blue-500 pl-4 uppercase tracking-wider text-lg">專案團隊資訊 (Project Team)</h2>
             <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-gray-50 p-6 rounded-2xl border border-gray-100">
                <div><label className="text-sm text-gray-500 block mb-2 font-bold">仲介公司</label><select className="w-full p-3 border rounded-lg text-base bg-white" value={projectTeam.agency} onChange={e => handleAgencyChange(e.target.value)}><option value="">-- 請選擇 --</option>{agencyDB && Object.keys(agencyDB).map(ag => <option key={ag} value={ag}>{ag}</option>)}</select></div>
                <div><label className="text-sm text-gray-500 block mb-2 font-bold">經紀人</label><select className="w-full p-3 border rounded-lg text-base bg-white" value={projectTeam.broker} onChange={e => setProjectTeam({...projectTeam, broker: e.target.value})} disabled={!projectTeam.agency}><option value="">{projectTeam.agency ? "-- 請選擇經紀人 --" : "-- 請先選擇公司 --"}</option>{projectTeam.agency && agencyDB[projectTeam.agency]?.map(bk => <option key={bk} value={bk}>{bk}</option>)}</select></div>
                <div className="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-4 border-t pt-4"><div><label className="text-sm text-gray-500 block mb-2 font-bold">開發業務 (人員)</label><input className="w-full p-3 border rounded-lg" value={projectTeam.developer} onChange={e=>setProjectTeam({...projectTeam, developer:e.target.value})} placeholder="開發人員姓名" /></div><div className="flex gap-2"><div className="flex-1"><label className="text-sm text-gray-500 block mb-2 font-bold">合約類型</label><select className="w-full p-3 border rounded-lg bg-white" value={projectTeam.developerType} onChange={e=>setProjectTeam({...projectTeam, developerType:e.target.value})}><option value="general">一般約</option><option value="exclusive">專任約</option></select></div><div className="flex-[2]"><label className="text-sm text-gray-500 block mb-2 font-bold">合約號碼</label><div className="flex gap-2"><input className="w-full p-3 border rounded-lg" value={projectTeam.developerNo} onChange={e=>setProjectTeam({...projectTeam, developerNo:e.target.value})} placeholder="合約編號" /><label className="cursor-pointer bg-gray-200 p-3 rounded-lg flex items-center justify-center text-gray-500 hover:bg-gray-300"><Camera className="w-5 h-5"/><input type="file" className="hidden" accept="image/*" onChange={(e)=>handleImageUploadGeneric(e.target.files[0], (res)=>setProjectTeam({...projectTeam, developerImg: res}))} /></label>{projectTeam.developerImg && <button onClick={()=>setPreviewImage(projectTeam.developerImg)} className="bg-blue-100 text-blue-600 p-3 rounded-lg font-bold text-xs whitespace-nowrap">查看</button>}</div></div></div></div>
                <div className="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-4 border-t pt-4"><div><label className="text-sm text-gray-500 block mb-2 font-bold">行銷業務 (人員)</label><input className="w-full p-3 border rounded-lg" value={projectTeam.marketer} onChange={e=>setProjectTeam({...projectTeam, marketer:e.target.value})} placeholder="行銷人員姓名" /></div><div className="flex gap-2"><div className="flex-1 pt-8 text-sm font-bold text-gray-400 text-center bg-gray-100 rounded-lg">斡旋 / 訂金</div><div className="flex-[2]"><label className="text-sm text-gray-500 block mb-2 font-bold">單據號碼</label><div className="flex gap-2"><input className="w-full p-3 border rounded-lg" value={projectTeam.marketerNo} onChange={e=>setProjectTeam({...projectTeam, marketerNo:e.target.value})} placeholder="單據編號" /><label className="cursor-pointer bg-gray-200 p-3 rounded-lg flex items-center justify-center text-gray-500 hover:bg-gray-300"><Camera className="w-5 h-5"/><input type="file" className="hidden" accept="image/*" onChange={(e)=>handleImageUploadGeneric(e.target.files[0], (res)=>setProjectTeam({...projectTeam, marketerImg: res}))} /></label>{projectTeam.marketerImg && <button onClick={()=>setPreviewImage(projectTeam.marketerImg)} className="bg-blue-100 text-blue-600 p-3 rounded-lg font-bold text-xs whitespace-nowrap">查看</button>}</div></div></div></div>
                <div className="md:col-span-2 border-t pt-4"><label className="text-sm text-gray-500 block mb-2 font-bold">承辦代書</label><input className="w-full p-3 border rounded-lg text-base" value={projectTeam.scrivener} onChange={e=>setProjectTeam({...projectTeam, scrivener:e.target.value})} placeholder="代書姓名/事務所" /></div>
             </div>
          </div>
        )}
        ...
        {activeTab === 'requisition' && (
          <div className="bg-white rounded-2xl shadow-sm border p-8 animate-fadeIn">
            <h2 className="font-bold text-gray-700 mb-6 flex items-center gap-2 border-l-4 border-blue-500 pl-4 uppercase tracking-wider text-lg">請款單管理 (Requisitions)</h2>
            <div className="bg-purple-50 p-6 rounded-2xl border border-purple-100 mb-8">
               <div className="grid grid-cols-1 md:grid-cols-6 gap-4 items-end">
                  <div><label className="text-xs font-bold text-purple-700 mb-1 block">日期</label><input type="date" className="w-full p-3 border rounded-lg bg-white" value={newReq.date} onChange={e=>setNewReq({...newReq, date:e.target.value})} /></div>
                  <div className="md:col-span-2">
                     <label className="text-xs font-bold text-purple-700 mb-1 block">標的物 (土地/建物)</label>
                     <select className="w-full p-3 border rounded-lg bg-white" value={newReq.target} onChange={e=>setNewReq({...newReq, target:e.target.value})}>
                        <option value="">-- 請選擇標的 --</option>
                        <optgroup label="土地">
                           {lands.map(l => <option key={l.id} value={`土地: ${l.sellers.map(s=>s.name).join('/')}`}>土地: {l.sellers.map(s=>s.name).join('/')}</option>)}
                        </optgroup>
                        <optgroup label="建物">
                           {buildings.map(b => <option key={b.id} value={`建物: ${b.address}`}>建物: {b.address}</option>)}
                        </optgroup>
                  ...
                  <button onClick={addRequisition} className="w-full p-3 bg-purple-600 text-white rounded-lg font-bold hover:bg-purple-700 shadow-md">新增</button>
                  <div className="md:col-span-6"><label className="text-xs font-bold text-purple-700 mb-1 block">款項明細說明</label><input className="w-full p-3 border rounded-lg bg-white" placeholder="輸入款項用途或說明..." value={newReq.details} onChange={e=>setNewReq({...newReq, details:e.target.value})} /></div>
               </div>
            </div>

            <div className="rounded-xl border overflow-hidden">
               <table className="w-full text-sm text-left">
                  <thead className="bg-purple-50 text-purple-900 text-xs font-bold uppercase">
                     <tr>
                        <th className="p-4 w-32">日期</th>
                        <th className="p-4 w-48">標的物</th>
                        <th className="p-4">明細</th>
                        <th className="p-4 w-32">股東</th>
                        <th className="p-4 w-32 text-right">金額</th>
                  ...
                  <tbody className="divide-y">
                     {requisitions.map(r => (
                        <tr key={r.id} className="hover:bg-gray-50">
                           ...
                           <td className="p-4 text-xs">{r.target}</td>
                           <td className="p-4">{r.details}</td>
                           <td className="p-4 font-bold text-gray-600">{r.shareholder}</td>
                           <td className="p-4 text-right font-mono font-bold text-purple-600">${Number(r.amount).toLocaleString()}</td>
                           <td className="p-4 text-center"><button onClick={()=>deleteRequisition(r.id)} className="text-gray-300 hover:text-red-500"><Trash2 className="w-4 h-4"/></button></td>
                        </tr>
                     ))}
                     {requisitions.length === 0 && <tr><td colSpan="6" className="p-8 text-center text-gray-300 italic">尚無請款紀錄</td></tr>}
                  </tbody>
               </table>
            </div>
          </div>
      ...
      <div className="hidden print:block print:p-8">
        ...
        {printConfig.requisition && requisitions.length > 0 && (
          <section className="mb-8 break-inside-avoid">
            <h2 className="text-lg font-bold border-b pb-2 mb-4">請款單明細</h2>
            <table className="w-full text-sm border-collapse border border-gray-300">
              <thead><tr className="bg-gray-100"><th className="border p-2">日期</th><th className="border p-2">標的物</th><th className="border p-2">明細</th><th className="border p-2">收/支股東</th><th className="border p-2 text-right">金額</th></tr></thead>
              <tbody>{requisitions.map(r => (<tr key={r.id}><td className="border p-2">{r.date}</td><td className="border p-2 text-xs">{r.target}</td><td className="border p-2">{r.details}</td><td className="border p-2">{r.shareholder}</td><td className="border p-2 text-right">${Number(r.amount).toLocaleString()}</td></tr>))}</tbody>
            </table>
          </section>
        )}
                  ...
                  return (
                    <div key={type} className="break-inside-avoid mb-4">
                      ...
                      <table className="w-full text-xs border-collapse border border-gray-300">
                        ...
                        <tbody>{subData.map(t => (<tr key={t.id}><td className="border p-2">{t.date}</td><td className="border p-2">{t.category}</td><td className="border p-2">{t.note}</td><td className="border p-2 text-right">${Number(t.amount).toLocaleString()}</td></tr>))}</tbody>
                        {/* ✅ 列印專用小計 */}
                        <tfoot className="bg-gray-50 border-t-2 border-gray-300 font-bold">
                            <tr>
                                ...
                                <td className="border p-2 text-right">
                                   <div>收: ${(subStats?.income||0).toLocaleString()}</div>
                                   <div>支: ${(subStats?.expense||0).toLocaleString()}</div>
                                   <div className="border-t border-gray-400 mt-1 pt-1">淨: ${net.toLocaleString()}</div>
                                </td>
                            </tr>
                        </tfoot>
                      </table>
