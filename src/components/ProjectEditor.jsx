        ...
        {activeTab === 'land' && (
           <div className="space-y-6 animate-fadeIn">
              <div className="bg-gradient-to-r from-blue-600 to-blue-800 p-6 rounded-3xl text-white shadow-xl mb-6">
                 <h3 className="text-lg font-black mb-4 flex items-center gap-2"><Map className="w-6 h-6"/> 全案土地總結算</h3>
              ...
              {showBuildingForm && (
                <div className="bg-white p-8 rounded-3xl shadow-xl border border-orange-200 animate-fadeIn">
                   ...
                   <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                     ...
                     <div><label className="text-sm text-gray-500 block mb-2 font-bold">使用執照號碼</label><div className="flex gap-2"><input placeholder="使照號碼" className="flex-1 w-full p-3 border rounded-lg text-base" value={tempBuilding.license} onChange={e => setTempBuilding({...tempBuilding, license: e.target.value})} /><label className="cursor-pointer bg-gray-100 hover:bg-gray-200 p-3 rounded-lg flex items-center gap-2 text-xs font-bold text-gray-500"><Camera className="w-4 h-4"/>{tempBuilding.licenseImage ? "已存" : "圖檔"}<input type="file" className="hidden" accept="image/*" onChange={(e)=>handleImageUploadGeneric(e.target.files[0], (res)=>setTempBuilding({...tempBuilding, licenseImage: res}))} /></label>{tempBuilding.licenseImage && <button onClick={()=>setTempBuilding({...tempBuilding, licenseImage: null})} className="bg-red-50 text-red-500 p-3 rounded-lg"><X className="w-4 h-4"/></button>}</div></div>
                     <div><label className="text-sm text-gray-500 block mb-2 font-bold">建物建號</label><div className="flex gap-2"><input placeholder="建號" className="flex-1 w-full p-3 border rounded-lg text-base" value={tempBuilding.buildNumber} onChange={e => setTempBuilding({...tempBuilding, buildNumber: e.target.value})} /><label className="cursor-pointer bg-gray-100 hover:bg-gray-200 p-3 rounded-lg flex items-center gap-2 text-xs font-bold text-gray-500"><Camera className="w-4 h-4"/>{tempBuilding.buildNoImage ? "已存" : "圖檔"}<input type="file" className="hidden" accept="image/*" onChange={(e)=>handleImageUploadGeneric(e.target.files[0], (res)=>setTempBuilding({...tempBuilding, buildNoImage: res}))} /></label>{tempBuilding.buildNoImage && <button onClick={()=>setTempBuilding({...tempBuilding, buildNoImage: null})} className="bg-red-50 text-red-500 p-3 rounded-lg"><X className="w-4 h-4"/></button>}</div></div>
        ...
        {activeTab === 'handover' && (
          <div className="bg-white rounded-2xl shadow-sm border p-8 animate-fadeIn">
             <div className="border-b pb-6 mb-6">
                ...
                <div className="flex items-center gap-4 bg-yellow-50 p-4 rounded-xl border border-yellow-100 mb-6"><label className="font-bold text-yellow-800 flex items-center gap-2"><CalendarIcon className="w-5 h-5"/> 點交日期</label><input type="date" className="p-2 border rounded-lg bg-white outline-none focus:ring-2 focus:ring-yellow-400" value={handoverData.handoverDate || ""} onChange={(e)=>setHandoverData({...handoverData, handoverDate: e.target.value})} /></div>
                <div className="bg-gray-50 p-4 rounded-xl text-sm text-gray-600 space-y-2 font-mono"><p><span className="font-bold text-gray-400">地籍地號：</span> {allLotNumbers || "無資料"}</p><p><span className="font-bold text-gray-400">建物資訊：</span> {allBuildingInfo || "無資料"}</p></div>
             </div>
