...
const ProjectEditor = ({ initialData, onSave, onBack }) => {
  ...
  const handleLandItemChange = (idx, field, value) => {
    const newItems = [...tempLand.items]; newItems[idx][field] = value;
    if (['areaM2', 'shareNum', 'shareDenom', 'pricePerPing'].includes(field)) {
      const area = Number(newItems[idx].areaM2)||0, num = Number(newItems[idx].shareNum)||0, denom = Number(newItems[idx].shareDenom)||1, price = Number(newItems[idx].pricePerPing)||0;
      const hM2 = area * (num / denom); newItems[idx].subtotal = Math.round(toPing(hM2) * price).toString();
    }
  ...
  const saveLand = () => {
    ...
    tempLand.items.forEach(item => { const hM2 = Number(item.areaM2)*(Number(item.shareNum)/Number(item.shareDenom)); totalM2+=hM2; totalPingSum+=toPing(hM2); totalPriceSum+=(Number(item.subtotal)||0); });
  ...
  const TransactionTable = ({ data, typeLabel, colorTheme, subStats }) => {
    ...
    return (
        ...
        <div className="p-4 border-b bg-gray-50/50 flex justify-between items-center">
            <h4 className="font-black text-gray-700 flex items-center gap-2 text-lg">{typeLabel}</h4>
            <span className="text-xs font-bold text-gray-400 bg-white px-2 py-1 rounded border">共 {data.length} 筆</span>
        </div>
