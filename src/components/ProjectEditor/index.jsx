...
import { exportMasterCSV, toROCDate, toPing } from '../../utils/helpers'; 
...
const ProjectEditor = ({ initialData, onSave, onBack, agencyDB }) => {
  ...
  const [handoverData, setHandoverData] = useState(
      Array.isArray(initialData.handoverData) ? initialData.handoverData : (initialData.handoverData ? [initialData.handoverData] : [])
  );
  ...
  // ✅ 新增：可見的交屋單戶號
  const [visibleHandoverUnits, setVisibleHandoverUnits] = useState([]);
  ...
  const buildingGrandTotal = useMemo(() => {
    let totalAreaM2 = 0, totalMoney = 0;
    buildings.forEach(b => { totalAreaM2 += Number(b.areaM2) || 0; totalMoney += Number(b.totalPrice) || 0; });
    return { m2: totalAreaM2.toFixed(3), ping: toPing(totalAreaM2).toFixed(3), price: totalMoney };
  }, [buildings]);
  ...
  // ✅ 預設全選所有交屋單
  useEffect(() => { 
      if (handoverData && handoverData.length > 0 && visibleHandoverUnits.length === 0) {
          setVisibleHandoverUnits(handoverData.map(h => h.unit));
      }
  }, [handoverData]);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      const rawData = { 
          ...
          projectTeam: projectTeams, buyers, lands, buildings, transactions, 
          handoverData, requisitions, shareholders, 
          updatedAt: new Date().toISOString() 
      };
      onSave(deepSanitize(rawData));
    }, 1500);
    return () => clearTimeout(timer);
  }, [projectName, projectZone, projectSite, projectTeams, buyers, lands, buildings, transactions, handoverData, requisitions, shareholders]);
  ...
  // ✅ 處理交屋單勾選
  const toggleHandoverVisibility = (unit) => {
      if (visibleHandoverUnits.includes(unit)) setVisibleHandoverUnits(visibleHandoverUnits.filter(u => u !== unit));
      else setVisibleHandoverUnits([...visibleHandoverUnits, unit]);
  };
    ...
    <div className="animate-fadeIn pb-24 text-base app-wrapper">
       <div className="print:hidden">
          ...
          <div className="flex gap-3 mb-8 overflow-x-auto no-scrollbar">
            ...
            <button onClick={() => setActiveTab('land')} className={`flex items-center gap-2 px-6 py-3 rounded-xl font-bold whitespace-nowrap transition-all duration-200 text-sm ${activeTab === 'land' ? 'bg-blue-600 text-white shadow-md' : 'bg-white text-gray-600 hover:bg-gray-100 border'}`}><Map className="w-5 h-5" /> 土地資料</button>
            <button onClick={() => setActiveTab('building')} className={`flex items-center gap-2 px-6 py-3 rounded-xl font-bold whitespace-nowrap transition-all duration-200 text-sm ${activeTab === 'building' ? 'bg-blue-600 text-white shadow-md' : 'bg-white text-gray-600 hover:bg-gray-100 border'}`}><Home className="w-5 h-5" /> 建物資料</button>
          ...
          {activeTab === 'building' && <BuildingSection buildings={buildings} setBuildings={setBuildings} sortedBuildings={sortedBuildings} setPreviewImage={setPreviewImage} transactions={transactions} setTransactions={setTransactions} handleImageUploadGeneric={handleImageUploadGeneric} buildingGrandTotal={buildingGrandTotal} />}
          {activeTab === 'requisition' && <RequisitionSection requisitions={requisitions} setRequisitions={setRequisitions} lands={lands} buildings={buildings} handleImageUploadGeneric={handleImageUploadGeneric} setPreviewImage={setPreviewImage} shareholders={shareholders} setShareholders={setShareholders} />}
          {/* ✅ 傳遞 lands, buildings 給 Handover */}
          {activeTab === 'handover' && <HandoverSection handoverData={handoverData} setHandoverData={setHandoverData} lands={lands} buildings={buildings} />}
       ...
       <PrintView 
          projectName={projectName} printConfig={printConfig} projectTeams={sortedTeams} 
          landGrandTotal={landGrandTotal} buildingGrandTotal={buildingGrandTotal} 
          buyers={buyers} sortedBuyers={sortedBuyers} 
          lands={lands} visibleLandIds={visibleLandIds} 
          buildings={buildings} sortedBuildings={sortedBuildings} 
          handoverData={handoverData} visibleHandoverUnits={visibleHandoverUnits} // ✅ 傳遞 visibleHandoverUnits
          allLotNumbers={allLotNumbers} allBuildingInfo={allBuildingInfo} 
          visibleLedgers={visibleLedgers} groupedTransactions={groupedTransactions} 
          stats={stats} requisitions={requisitions} groupedRequisitions={groupedRequisitions} 
       />
