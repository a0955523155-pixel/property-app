...
import { Plus, Trash2, Camera, FileText, Receipt, Building2, Map, DollarSign, PlusCircle, X } from 'lucide-react';
...
const TeamSection = ({ 
  ...
  const addTeamMember = () => setProjectTeams([
      ...
      { 
          ...
          unit: "", 
          agency: "", broker: "", 
          developer: "", developerType: "", developerNo: "", developerImages: [], // ✅ 支援多圖
          devServiceFee: "", devInvoices: [], 
          marketer: "", marketerNo: "", marketerImages: [], // ✅ 支援多圖
          marServiceFee: "", marInvoices: [], 
          scrivener: "" 
      }
  ]);
  ...
  const addInvoiceRow = (teamId, role) => {
      ...
      setProjectTeams(projectTeams.map(t => {
          if (t.id === teamId) {
              ...
              return { ...t, [field]: [...currentArray, { id: Date.now(), date: '', invoiceNo: '', details: '', amount: '' }] };
          }
          return t;
      }));
  };
    ...
    <div className="bg-white rounded-2xl shadow-sm border p-8 animate-fadeIn">
      ...
      <div className="space-y-6">
        {projectTeams.map((team, index) => {
          // ✅ 自動相容舊版單一圖片，並轉為陣列
          const devImgs = team.developerImages || (team.developerImg ? [team.developerImg] : []);
          const marImgs = team.marketerImages || (team.marketerImg ? [team.marketerImg] : []);

          // ✅ 計算總合計
          const devTotal = (team.devInvoices || []).reduce((sum, inv) => sum + (Number(inv.amount) || 0), 0);
          const marTotal = (team.marInvoices || []).reduce((sum, inv) => sum + (Number(inv.amount) || 0), 0);
            ...
            <div key={team.id || index} className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-gray-50 p-6 rounded-2xl border border-gray-100 relative group">
              ...
              <div className="md:col-span-2 bg-white p-4 rounded-xl border border-blue-100 shadow-sm">
                  ...
                  <div className="bg-blue-50 p-3 rounded-lg mb-4">
                      ...
                      {/* ✅ 多張照片上傳與顯示 */}
                      <div className="flex flex-wrap items-center gap-2 mb-3 bg-white p-2 rounded border border-blue-100">
                          <label className="cursor-pointer bg-blue-100 text-blue-700 hover:bg-blue-200 px-3 py-1.5 rounded text-xs font-bold transition flex items-center gap-1 shadow-sm">
                              <Camera className="w-4 h-4"/> 上傳合約照片
                              <input type="file" className="hidden" accept="image/*" onChange={(e)=>handleImageUploadGeneric(e.target.files[0], (res)=>updateTeamMember(team.id, 'developerImages', [...devImgs, res]))} />
                          </label>
                          {devImgs.map((img, i) => (
                              <div key={i} className="flex items-center gap-1 bg-gray-100 border border-gray-200 px-2 py-1 rounded shadow-sm">
                                  <button onClick={()=>setPreviewImage(img)} className="text-xs font-bold text-gray-700 hover:text-blue-600 hover:underline">照片 {i+1}</button>
                                  <button onClick={()=>updateTeamMember(team.id, 'developerImages', devImgs.filter((_, idx)=>idx!==i))} className="text-red-400 hover:text-red-600"><X className="w-3 h-3"/></button>
                              </div>
                          ))}
                      </div>
                  ...
                  <div className="border-t border-blue-100 pt-4 mt-2">
                      ...
                      <div className="space-y-2">
                          {(team.devInvoices || []).map((inv) => (
                              <div key={inv.id} className="grid grid-cols-1 md:grid-cols-12 gap-2 bg-gray-50 p-2 rounded border border-gray-200 items-center">
                                  <div className="md:col-span-3"><input type="date" className="w-full p-1.5 border rounded text-xs outline-none focus:border-blue-400" value={inv.date || ''} onChange={e=>updateInvoiceRow(team.id, 'dev', inv.id, 'date', e.target.value)} title="款項日期"/></div>
                                  <div className="md:col-span-3"><input type="text" className="w-full p-1.5 border rounded text-xs outline-none focus:border-blue-400" value={inv.invoiceNo || ''} onChange={e=>updateInvoiceRow(team.id, 'dev', inv.id, 'invoiceNo', e.target.value)} placeholder="發票號碼"/></div>
                                  ...
                                  <div className="md:col-span-2 flex items-center gap-1">
                                      <input type="number" className="w-full p-1.5 border rounded text-xs outline-none focus:border-blue-400 font-bold text-blue-600 text-right" value={inv.amount || ''} onChange={e=>updateInvoiceRow(team.id, 'dev', inv.id, 'amount', e.target.value)} placeholder="$ 金額"/>
                      ...
                      <div className="flex justify-between items-center mt-3">
                          <button onClick={()=>addInvoiceRow(team.id, 'dev')} className="flex items-center gap-1 text-xs font-bold text-blue-600 hover:text-blue-800 bg-blue-50 hover:bg-blue-100 px-3 py-1.5 rounded transition"><PlusCircle className="w-4 h-4"/> 新增明細</button>
                          <div className="text-sm font-black text-blue-800 bg-blue-100 px-3 py-1.5 rounded border border-blue-200">合計: ${devTotal.toLocaleString()}</div>
                      </div>
                  </div>
              </div>
              ...
              <div className="md:col-span-2 bg-white p-4 rounded-xl border border-purple-100 shadow-sm">
                  ...
                  <div className="bg-purple-50 p-3 rounded-lg mb-4">
                      <div className="flex justify-between items-center mb-2">
                          <label className="text-xs font-bold text-purple-700 flex items-center gap-1"><Receipt className="w-4 h-4"/> 已登錄單據</label>
                      </div>
                      {(team.marketerNo) ? (
                      ...
                      {/* ✅ 多張照片上傳與顯示 */}
                      <div className="flex flex-wrap items-center gap-2 mb-3 bg-white p-2 rounded border border-purple-100">
                          <label className="cursor-pointer bg-purple-100 text-purple-700 hover:bg-purple-200 px-3 py-1.5 rounded text-xs font-bold transition flex items-center gap-1 shadow-sm">
                              <Camera className="w-4 h-4"/> 上傳單據照片
                              <input type="file" className="hidden" accept="image/*" onChange={(e)=>handleImageUploadGeneric(e.target.files[0], (res)=>updateTeamMember(team.id, 'marketerImages', [...marImgs, res]))} />
                          </label>
                          {marImgs.map((img, i) => (
                              <div key={i} className="flex items-center gap-1 bg-gray-100 border border-gray-200 px-2 py-1 rounded shadow-sm">
                                  <button onClick={()=>setPreviewImage(img)} className="text-xs font-bold text-gray-700 hover:text-purple-600 hover:underline">照片 {i+1}</button>
                                  <button onClick={()=>updateTeamMember(team.id, 'marketerImages', marImgs.filter((_, idx)=>idx!==i))} className="text-red-400 hover:text-red-600"><X className="w-3 h-3"/></button>
                              </div>
                          ))}
                      </div>
                  ...
                  <div className="border-t border-purple-100 pt-4 mt-2">
                      ...
                      <div className="space-y-2">
                          {(team.marInvoices || []).map((inv) => (
                              <div key={inv.id} className="grid grid-cols-1 md:grid-cols-12 gap-2 bg-gray-50 p-2 rounded border border-gray-200 items-center">
                                  <div className="md:col-span-3"><input type="date" className="w-full p-1.5 border rounded text-xs outline-none focus:border-purple-400" value={inv.date || ''} onChange={e=>updateInvoiceRow(team.id, 'mar', inv.id, 'date', e.target.value)} title="款項日期"/></div>
                                  <div className="md:col-span-3"><input type="text" className="w-full p-1.5 border rounded text-xs outline-none focus:border-purple-400" value={inv.invoiceNo || ''} onChange={e=>updateInvoiceRow(team.id, 'mar', inv.id, 'invoiceNo', e.target.value)} placeholder="發票號碼"/></div>
                                  ...
                                  <div className="md:col-span-2 flex items-center gap-1">
                                      <input type="number" className="w-full p-1.5 border rounded text-xs outline-none focus:border-purple-400 font-bold text-purple-600 text-right" value={inv.amount || ''} onChange={e=>updateInvoiceRow(team.id, 'mar', inv.id, 'amount', e.target.value)} placeholder="$ 金額"/>
                      ...
                      {/* 顯示合計 */}
                      <div className="flex justify-between items-center mt-3">
                          <button onClick={()=>addInvoiceRow(team.id, 'mar')} className="flex items-center gap-1 text-xs font-bold text-purple-600 hover:text-purple-800 bg-purple-50 hover:bg-purple-100 px-3 py-1.5 rounded transition"><PlusCircle className="w-4 h-4"/> 新增明細</button>
                          <div className="text-sm font-black text-purple-800 bg-purple-100 px-3 py-1.5 rounded border border-purple-200">合計: ${marTotal.toLocaleString()}</div>
                      </div>
                  </div>
              </div>
